...
const size = bytes => {if(bytes==null||!Number.isFinite(Number(bytes))||Number(bytes)<0)return '—';const units=['B','KB','MB','GB','TB','PB'];let value=Number(bytes),unit=0;while(value>=1024&&unit<units.length-1){value/=1024;unit++}return `${unit?value.toFixed(1):value} ${units[unit]}`};
...
function ruleSourceName(source){return source==='local'?'本机镜像':source==='upstream'?'上游源':'沿用原有方式（兼容）'}
function ruleSourceOptions(items=[]){return ['local','upstream'].map(id=>{const item=items.find(value=>value.id===id);return{id,label:ruleSourceName(id),available:!!item&&item.available!==false,reason:item?.reason||item?.description||'',...item}})}
function renderRuleSourceControl(id,{value='legacy',options=[],reason='',disabled=false,onchange,showLegacy=false,legacyValue='legacy',legacyLabel='沿用原有方式（兼容）'}={}){
  const box=$(id);if(!box)return;
  box.dataset.value=value;
  const choices=ruleSourceOptions(options),compat=!['local','upstream'].includes(value);
  box.innerHTML=`<fieldset class="rule-source-control"><legend>分流规则来源</legend><div class="rule-source-segments">${choices.map(choice=>`<label class="rule-source-choice${disabled||!choice.available?' unavailable':''}"><input type="radio" name="${escapeHTML(id)}" value="${choice.id}"${value===choice.id?' checked':''}${disabled||!choice.available?' disabled':''}><span>${ruleSourceName(choice.id)}${!choice.available&&choice.reason?`<small>${escapeHTML(choice.reason)}</small>`:''}</span></label>`).join('')}</div>${compat||showLegacy?`<label class="rule-source-compat"><input type="radio" name="${escapeHTML(id)}" value="${escapeHTML(compat?value:legacyValue)}"${compat?' checked':''}${disabled?' disabled':''}><span>${escapeHTML(legacyLabel)}</span></label>`:''}${reason?`<p class="field-hint">${escapeHTML(reason)}</p>`:''}</fieldset>`;
  box.querySelectorAll('input').forEach(input=>input.onchange=()=>{if(!input.checked||input.disabled)return;box.dataset.value=input.value;onchange?.(input.value)});
}
function ruleSourceValue(id){return $(id)?.dataset.value||'legacy'}
function ruleSourceSummary(metadata){
  if(!metadata)return '尚未生成来源记录；请以实际输出为准。';
  const source=metadata.actual_source||metadata.source,version=metadata.version||metadata.revision||metadata.rule_revision;
  const details=[`实际来源：${ruleSourceName(source)}`,version&&`版本：${version}`];
  if(metadata.count!=null)details.push(`有效规则：${Number(metadata.count).toLocaleString()} 条`);
  if(metadata.uncovered!=null)details.push(`未覆盖：${Array.isArray(metadata.uncovered)?metadata.uncovered.length:Number(metadata.uncovered)} 项`);
  if(metadata.note)details.push(metadata.note);
  return details.filter(Boolean).join(' · ');
}
function verifiedRuleComparison(local,upstream){
  if(!local||!upstream)return{comparable:false,message:'先分别打开本机镜像与上游原文件，再比较已读取的版本。'};
  const localVerified=local.verified===true||local.mirrored===true;
  const upstreamVerified=upstream.verified===true||upstream.upstream_verified===true||upstream.hash_comparable===true||upstream.can_compare===true;
  if(!localVerified||local.mirrored===false||local.local_error||local.verified===false||(local.health&&local.health!=='verified')||!upstreamVerified||upstream.verified===false||(upstream.health&&upstream.health!=='verified')||upstream.last_error||upstream.upstream_error||!local.sha256||!upstream.sha256)return{comparable:false,message:'原始文件尚未完成双侧校验，当前不可比较。'+(local.local_error?' 本机原因：'+local.local_error:'')+(upstream.last_error?' 上游原因：'+upstream.last_error:'')};
  return{comparable:true,equal:local.sha256===upstream.sha256,message:local.sha256===upstream.sha256?'已验证的两份原始文件 SHA-256 相同。':'已验证的两份原始文件 SHA-256 不同。'};
}
...
window.CoralBayResourceDrawer=(()=>{
  ...
  function renderComparison(){
    ...
    const comparison=verifiedRuleComparison(local,upstream);$('resourceStatus').textContent=comparison.message;$('resourceStatus').classList.toggle('bad',!comparison.comparable);
  ...
  async function load(){
    ...
    try{
      ...
      $('resourceMeta').innerHTML=metaRow('来源',source==='local'?'本机已发布':source==='upstream'?'上游预览':'版本差异')+metaRow(source==='diff'?'本地版本':'版本',source==='diff'?data.local_revision||'—':data.revision||'—')+(data.geo_revision?metaRow('关联 geo 版本',data.geo_revision):'')+(source==='diff'?metaRow('上游版本',data.upstream_revision)+metaRow('原始文件校验',data.comparable!==false&&typeof data.same==='boolean'?(data.same?'SHA-256 相同':'SHA-256 不同'):'缺少可比文件'):metaRow(source==='local'?'本地原始文件':'上游文件状态',data.verified===false?(data.health==='corrupt'?'原始文件损坏':data.health==='legacy_only'?'仅兼容缓存，未校验原文件':'原始文件尚未校验'):source==='local'&&data.mirrored===false?'未镜像（仅兼容缓存）':data.cached===false?(source==='local'?'本地缺失':'尚未获取'):'可用'))+metaRow('格式 / 行为',[data.format,data.behavior].filter(Boolean).join(' / ')||'—')+metaRow('文件大小',size(data.bytes))+metaRow('SHA-256',data.sha256||'—')+metaRow(source==='local'?'本地更新时间':'上游预览缓存时间',resourceDate(data.updated_at))+metaRow('最近检查',resourceDate(data.checked_at));
    ...
    }catch(error){if(id!==request)return;delete snapshots[source];$('resourceStatus').textContent=`${source==='local'?'本地读取':source==='upstream'?'上游预览':'差异比较'}失败：${error.message}`;$('resourceStatus').classList.add('bad');$('resourcePage').textContent='';}
...
async function loadRules(){
  ...
  try{
    ...
    $('ruleRows').innerHTML=(data.rules||[]).map(rule=>{const local=resourceURL(rule.local_url||rule.mirror_url),upstream=resourceURL(rule.original_url);return `<tr><td><div class="rule-name"><img src="${escapeHTML(resourceURL(rule.icon_url))}" alt=""><div><strong>${escapeHTML(rule.name)}</strong><br><small>${escapeHTML(rule.path)}</small></div></div></td><td>${escapeHTML(rule.behavior)}<br><small>${escapeHTML(rule.format)}</small></td><td class="${rule.cached?'ok':'warning'}">${rule.cached?'● 本机镜像可用':'○ 未同步或需修复'}</td><td>${size(rule.bytes)}</td><td><div class="source-file-links"><div>${rule.cached&&local?resourceLink(local,'本机镜像'):'<span class="warning">本机镜像 · 未同步</span>'}${!rule.cached?'<button class="link-button" type="button" data-repair-666>同步资源</button>':''}${local?`<button class="link-button" type="button" data-source-copy="${escapeHTML(local)}">复制地址</button>`:''}</div><div>${upstream?resourceLink(upstream,'上游原文件'):'<span class="muted">上游原文件 · 无等价原件</span>'}</div><button class="link-button" type="button" data-detail="${escapeHTML(rule.path)}" data-name="${escapeHTML(rule.name)}">详情</button></div></td></tr>`}).join('');
    ...
    $('ruleRows').querySelectorAll('[data-repair-666]').forEach(button=>button.onclick=()=>operate666('sync'));
    $('ruleRows').querySelectorAll('[data-source-copy]').forEach(button=>button.onclick=async()=>{try{await navigator.clipboard.writeText(button.dataset.sourceCopy);consoleNotice('本机镜像地址已复制；未同步资源须同步成功后才能下载。')}catch{consoleNotice('剪贴板不可用，请从详情中手动复制地址。')}});
  }catch(error){$('ruleRows').innerHTML=`<tr><td colspan="5" class="bad">规则目录加载失败：${escapeHTML(error.message)}</td></tr>`;throw error;}
}
let operation666Busy=false;
async function operate666(kind){
  if(operation666Busy)return;operation666Busy=true;const controls=()=>[$('ruleCheckUpstream'),$('ruleSyncLocal'),...document.querySelectorAll('[data-repair-666]')];controls().forEach(button=>button.disabled=true);
  ...
  finally{operation666Busy=false;controls().forEach(button=>button.disabled=false)}
}

let templateItems = [];
let templatePreviewRequest = 0;
let templateSourceSelection='default';
function setArtifactLink(id,url){const element=$(id),safe=resourceURL(url);if(safe)element.href=safe;else element.removeAttribute('href');element.setAttribute('aria-disabled',String(!safe));element.classList.toggle('unavailable',!safe)}
function localArtifactPreviewURL(url){const safe=resourceURL(url);if(!safe)return '';const path=new URL(safe).pathname;return /^\/(?:_templates\/|_rule-templates\/666os\/)/.test(path)?path:safe}
function selectTemplate() {
  const selected=templateItems.find(item=>item.id===$('clientTemplate').value);if(!selected)return;
  const original=$('templateVariant').value==='original',available=original?[]:(selected.rule_source_options||[]),value=original?'default':templateSourceSelection;
  const ruleSource=available.find(item=>item.id===value),explicit=['local','upstream'].includes(value),blocked=explicit&&(!ruleSource||ruleSource.available===false);
  const reason=original?'Perfect Panel 原始版保持原样；改造版与原始版独立选择。':selected.capability==='nodes-only'?'仅节点输出，不适用分流规则来源。':available.length?'选择只改变模板中规则文件的下载来源；客户端版本与模板版本保持独立。':['clash','mihomo','openclash','stash'].includes(selected.id)?'当前尚未发布来源变体，请先同步 666OS；现有模板地址继续兼容。':'此格式使用本机派生或转换资源，没有等价上游原文件。';
  renderRuleSourceControl('templateRuleSource',{value,options:available,reason,showLegacy:blocked,legacyValue:'default',legacyLabel:'现有模板地址（兼容）',onchange:next=>{templateSourceSelection=next;selectTemplate()}});
  ...
  const sourceLabel=ruleSource?ruleSourceName(ruleSource.id):selected.rule_sources;
...
let mihomoCatalog=null,mihomoLoading=false,mihomoPreviewRequest=0;
function selectedMihomoSource(){const source=ruleSourceValue('mihomoRuleSource');return source==='legacy'?mihomoCatalog?.legacy:mihomoCatalog?.source_options?.find(item=>item.id===source)}
async function previewMihomoPro(){
  const item=selectedMihomoSource(),kind=$('mihomoPreviewKind').value,url=item?.available===false?'':item?.[kind==='config'?'config_url':'overwrite_url'],request=++mihomoPreviewRequest;
  if(!url){$('mihomoPreviewRefresh').disabled=false;$('mihomoPreview').textContent=item?.reason||'当前来源尚未就绪，没有对应文件可预览。';return}
  $('mihomoPreview').textContent='正在读取所选文件…';$('mihomoPreviewRefresh').disabled=true;
  try{const response=await fetch(localArtifactPreviewURL(url),{cache:'no-store'});if(!response.ok)throw new Error(`HTTP ${response.status}`);const content=await response.text();if(request===mihomoPreviewRequest)$('mihomoPreview').textContent=content}catch(error){if(request===mihomoPreviewRequest)$('mihomoPreview').textContent='预览失败：'+error.message}finally{if(request===mihomoPreviewRequest)$('mihomoPreviewRefresh').disabled=false}
}
function renderMihomoPro(){
...
async function loadMihomoPro(force=false){
  ...
  try{mihomoCatalog=await json('/api/resources/666os/mihomopro');renderMihomoPro();previewMihomoPro();$('mihomoStatus').textContent=mihomoCatalog.local_error||'';$('mihomoStatus').classList.toggle('hidden',!mihomoCatalog.local_error);$('mihomoStatus').classList.toggle('bad',!!mihomoCatalog.local_error)}catch(error){$('mihomoStatus').textContent='读取失败：'+error.message;$('mihomoStatus').classList.add('bad')}finally{mihomoLoading=false;$('mihomoRefresh').disabled=false}
}
...
function renderConversions() {
  ...
  $('conversionRows').innerHTML = items.map(item => `<tr class="${item.entries===0?'empty-rule':''}"><td><strong>${escapeHTML(item.id.replace(/^(site|ip)-/,''))}</strong>${item.entries===0?'<br><small class="warning">无公开可读源 · 当前零覆盖</small>':''}</td><td>${item.kind==='site'?'域名':'IP/CIDR'}</td><td>${Number(item.entries).toLocaleString()}</td><td><code>${escapeHTML(item.source)}</code><br><small class="muted">本机派生规则；无等价上游原文件</small></td><td><a href="${escapeHTML(item.list_url)}" target="_blank" rel="noreferrer">RULE-SET</a> · <a href="${escapeHTML(item.singbox_url)}" target="_blank" rel="noreferrer">sing-box JSON</a> · <button class="link-button" data-copy-url="${escapeHTML(item.list_url)}">复制链接</button></td></tr>`).join('') || '<tr><td colspan="5" class="muted">没有匹配的转换产物</td></tr>';
...
let subscriptionPresets=[],subscriptionFormInitialized=false,subscriptionFormRestored=false,subscriptionSourceChoice='legacy';
...
function subscriptionNodesOnly(){return $('subListOnly').checked||['ss','ssr','trojan','mixed','v2ray'].includes($('subTarget').value)}
function effectiveSubscriptionPreset(){const config=$('subConfig').value.trim();return subscriptionPresets.find(preset=>config?config===preset.local_url||config===preset.original_url:preset.id==='none')}
function subscriptionRuleOptions(preset=effectiveSubscriptionPreset()){
  return ruleSourceOptions(preset?.rule_source_options||[]).map(option=>{const targetAllowed=!option.targets||option.targets.includes($('subTarget').value);return {...option,available:option.available&&targetAllowed,reason:!targetAllowed?'此预设的来源切换目前仅支持 Mihomo / Clash Meta 与 Stash。':option.reason||preset?.rule_source_note||'此配置没有完整的上游规则映射，保持原有规则来源。'}});
}
function renderSubscriptionRuleSource(){
  if(!$('subRuleSource'))return;const preset=effectiveSubscriptionPreset(),nodesOnly=subscriptionNodesOnly(),options=subscriptionRuleOptions(preset),choice=options.find(item=>item.id===subscriptionSourceChoice),blocked=['local','upstream'].includes(subscriptionSourceChoice)&&!choice?.available;
  const reason=nodesOnly?'仅节点输出，不适用分流规则来源。':preset?.built_in?'生成时读取所选来源，规则内嵌到结果；此设置不改变节点订阅地址。':preset?.rule_source_note||'此配置没有完整的规则来源映射；配置文件镜像不代表其中所有规则都来自本机。';
  renderRuleSourceControl('subRuleSource',{value:subscriptionSourceChoice,options,disabled:nodesOnly,reason,showLegacy:blocked,legacyLabel:'沿用配置中的规则来源（兼容）',onchange:value=>{subscriptionSourceChoice=value;renderSubscriptionRuleSource()}});
  $('subRuleSourceHint').textContent=nodesOnly?'生成结果只包含节点，来源选项不会提交。':blocked?choice?.reason||'所选来源当前不可用，请选择兼容方式或更换预设。':subscriptionSourceChoice==='legacy'?'保持原有转换行为；未指定分流规则来源。':'待生成验证后显示实际版本、有效规则与未覆盖数量。';
}
function applySubscriptionPreset(){
  const preset=subscriptionPresets.find(item=>item.id===$('subPreset').value);if(!preset)return;
  if(preset.built_in){$('subPresetSource').value='local';$('subPresetSource').disabled=true;$('subConfig').value=preset.local_url;$('subPresetDetail').innerHTML='<span class="template-status adapted">● 内置 MihomoPro</span><span>使用内置配置结构；下方统计是现有配置文件的依赖情况。明确切换分流规则来源后，以生成结果中的实际统计为准。</span>'+presetDependencies(preset);renderSubscriptionRuleSource();return}
  $('subPresetSource').disabled=false;let local=$('subPresetSource').value==='local',fallback='';if(local&&!preset.cached){$('subPresetSource').value='original';local=false;fallback='<span class="warning">本机尚无该配置文件，已改用原链接；分流规则来源未自动切换。</span><br>'}
  $('subConfig').value=preset.id==='none'?'':local?preset.local_url:preset.original_url;
  const state=preset.id==='none'?'不使用远程配置':preset.cached?`<span class="ok">● 已缓存 · ${size(preset.bytes)}</span>`:'<span class="bad">○ 未缓存，仅可使用原链接</span>';
  $('subPresetDetail').innerHTML=`${fallback}${state} · 配置文件来源：${local?'本机配置文件镜像':'上游原链接'}${preset.updated_at?` · 更新于 ${escapeHTML(resourceDate(preset.updated_at))}`:''}${preset.error?`<br><span class="bad">最近同步错误：${escapeHTML(preset.error)}</span>`:''}<br><span class="muted">此选项仅控制 INI 配置文件的读取位置；其中嵌套的规则 URL 可能仍指向外部来源。</span>${presetDependencies(preset)}`;
  renderSubscriptionRuleSource();
}
function restoreSubscriptionPresetSelection(){const preset=effectiveSubscriptionPreset();$('subPreset').value=preset?.id||'none';$('subPresetSource').disabled=!!preset?.built_in;if(preset&&$('subConfig').value.trim()===(preset.local_url||''))$('subPresetSource').value='local';else $('subPresetSource').value='original';$('subPresetDetail').textContent='保留当前配置 URL 与分流规则来源；选择预设后才会替换配置文件。';renderSubscriptionRuleSource()}
async function loadSubscriptionPresets(){
  const data=await json('/api/admin/subscription-presets'),preserve=subscriptionFormInitialized||subscriptionFormRestored||!!$('subConfig').value.trim();subscriptionPresets=data.presets||[];
  const groups=[];for(const item of subscriptionPresets){let group=groups.find(value=>value.name===item.group);if(!group){group={name:item.group,items:[]};groups.push(group)}group.items.push(item)}
  $('subPreset').innerHTML=groups.map(group=>`<optgroup label="${escapeHTML(group.name)}">${group.items.map(item=>`<option value="${escapeHTML(item.id)}">${item.built_in?'◆':item.cached?'●':'○'} ${escapeHTML(item.name)}</option>`).join('')}</optgroup>`).join('');
  $('subPresetCache').textContent=`${data.cached||0} / ${data.total||0} 已缓存 · MihomoPro 内置`;$('subPreset').onchange=applySubscriptionPreset;$('subPresetSource').onchange=applySubscriptionPreset;
  if(preserve)restoreSubscriptionPresetSelection();else{applySubscriptionPreset();if(subscriptionRuleOptions().find(item=>item.id==='local')?.available)subscriptionSourceChoice='local';renderSubscriptionRuleSource()}
...
if ($('sync')) {
  ...
  $('conversionSearch').oninput=renderConversions; $('conversionKind').onchange=renderConversions;
	...
	$('subTarget').onchange=()=>{updateSubCompatibility();renderSubscriptionRuleSource()};$('subListOnly').onchange=renderSubscriptionRuleSource;$('subConfig').oninput=()=>{subscriptionFormRestored=true;restoreSubscriptionPresetSelection()};updateSubCompatibility();renderSubscriptionRuleSource();
  $('mihomoRefresh').onclick=()=>loadMihomoPro(true);$('mihomoPreviewKind').onchange=previewMihomoPro;$('mihomoPreviewRefresh').onclick=previewMihomoPro;
	$('generateSub').onclick=async()=>{const feedback=document.querySelector('.converter-actions .field-hint');try{$('generateSub').disabled=true;$('generateSub').textContent='正在拉取并验证…';feedback.textContent='正在由本机后端获取并解析订阅…';feedback.classList.remove('bad','ok');$('subResult').classList.add('hidden');const data=await json('/api/admin/subscription-link',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(subscriptionPayload())});$('subResultURL').textContent=data.url;$('subRuleSourceResult').textContent=ruleSourceSummary(data.rule_source_metadata)+(data.rule_source_metadata?.warnings?.length?' · '+data.rule_source_metadata.warnings.join('；'):'');$('subValidation').textContent=`✓ 已验证 ${data.node_count} 个可解析节点`;$('openSubResult').href=data.url;$('downloadSubResult').href=data.url;$('subQR').src=`/api/admin/subscription-qr?url=${encodeURIComponent(data.url)}`;$('subResult').classList.remove('hidden');feedback.textContent=`转换验证通过：${data.node_count} 个可解析节点`;feedback.classList.add('ok');usagePage=1;await loadSubscriptionUsage()}catch(error){feedback.textContent=`转换失败：${error.message}`;feedback.classList.add('bad')}finally{$('generateSub').disabled=false;$('generateSub').textContent='测试并生成'}};
