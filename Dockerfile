...
COPY go.mod go.sum ./
COPY *.go ./
COPY remote-configs.json routing_catalog.json expected-files.txt ./
COPY cmd ./cmd
...
RUN apk add --no-cache ca-certificates curl git tzdata util-linux

COPY expected-files.txt /app/expected-files.txt
COPY templates /app/templates
COPY assets /app/assets
COPY docs/reference/metacubex-meta-rules-dat-LICENSE /app/licenses/MetaCubeX-LICENSE
COPY docs/reference/metacubex-routing-source-notice.md /app/licenses/MetaCubeX-NOTICE.md
